...
export default function LendPage() {
  ...
  const fetchBooks = async (userId, tokenToUse) => {
    try {
      setLoading(true);
      const res = await fetch(`http://localhost:8080/api/lending/books/user?userId=${userId}`, {
        headers: {
  ...
  const handleReturn = async (lendingId) => {
    try {
      const res = await fetch(`http://localhost:8080/api/lending/return?lendingId=${lendingId}`, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
        },
